...
> `다른 일반 스레드`가 모두 `종료`되면 자신의 작업이 `끝나지 않았더라도` 같이 `종료`됨.  

일반 스레드의 작업을 돕는 `보조역할`을 수행  
`가비지 컬렉터`, `자동저장`, `화면 자동갱신` 등에 사용됨.  
무한루프, 조건문으로 실행 - 대기 - 실행 - ... 하도록 작성.  
...
![alt text](../../image/ThreadStates_2.PNG)  
`NEW` : 스레드가 생성되고 실행되지 않은 상태.    

`RUNNABLE` : 스레드가 실행되고 있는 상태.  
다른 스레드들과 실행, 실행대기를 번갈아가며 교차함. 
...
`notity()` : 같은 객체를 공유하는 다른 스레드를 하나 깨우기  
`notifyAll()` : 같은 객체를 공유하는 모든 스레드 깨우기  
