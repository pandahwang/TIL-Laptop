...
- Interface 내에 있는 메서드는 `public abstract`를 <span style = "color:yellow">생략</span>  해도 됨.  
(모두 public abstract 메서드이기 때문에 안 써도 컴파일러가 알아서 그렇게 생각함.)

- `Interface의 조상`은 <span style = "color:yellow">Interface</span> 만 가능함. `Object`는 조상이 될 수 없음.  

- `다중 상속 가능`. 이유: 추상 메서드는 몸통이 없기 때문에 충돌하지 않음.  

- Interface의 완성(상속)은 `implements`를 사용.  
<span style = "color:red">정의된 모든 추상 메서드를 구현해야 함.</span>    
일부만 구현할 경우에는 클래스 앞에 `abstract`를 붙임.

- Interface 내에 있는 메서드만 사용 가능.  
Interface 메서드의 인자(매개변수)가 참조변수인 경우엔, Interface를 상속받아 구현한 클래스의 인스턴스만 사용 가능. 
