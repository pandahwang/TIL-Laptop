...
```
## 예외 전가 (throws)  
> `throws` 키워드를 통해 예외 처리를 상위 메서드에 전가함.  

- 예외 발생 구문을 직접 try-catch문으로 처리하는 것이 아니라,  
예외를 throws하면 `해당 구문을 호출한 메서드`(상위 메서드)가 처리해야 함.  
...
```
그러면 main 함수마저 전가 throw 해버리면 어떻게 되냐??  
JVM에 전가가 되는데, 이렇게 되면 프로그램이 종료되어버림.  
